<?php
class ShopAccessor extends Accessor {
	...
	function getShopCategoryList() {
		...
		$sql = "SELECT * FROM shop_category_t ORDER BY disp_order ";
		$result = $con->getAll($sql, null, DB_FETCHMODE_ASSOC);
		if (DB :: isError($result)) {
			$con->disconnect();
			return "業種一覧取得時エラー:" . DB :: errorMessage($result);
		}
	...
	// 大エリア一覧取得
	function getShopAreaLargeList() {
		$con = DB :: connect($this->dsn);
		if (DB :: isError($con)) {
			return "DB接続エラー:" . DB :: errorMessage($con);
		}

		$sql = "SELECT * FROM shop_area_large_t ORDER BY disp_order ";
		$result = $con->getAll($sql, null, DB_FETCHMODE_ASSOC);
		if (DB :: isError($result)) {
			$con->disconnect();
			return "大エリア一覧取得時エラー:" . DB :: errorMessage($result);
		}
